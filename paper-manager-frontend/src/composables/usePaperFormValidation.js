...
export function usePaperFormValidation(form) {
  ...
  const validationRules = {
    ...
    publication_date: {
      ...
      minLength: 2,
      maxLength: 50,
      minLengthMessage: '通讯作者姓名至少需要2个字符',
      maxLengthMessage: '通讯作者姓名不能超过50个字符'
    },
    file: {
      custom: (value) => {
        if (value) {
  ...
  const validateField = (field, value, formData = {}) => {
    ...
    // 条件必填验证
    if (rule.requiredIf && rule.requiredIf(formData) && (!value || (typeof value === 'string' && !value.trim()))) {
      errors.value[field] = rule.message
      return false
    }
    ...
    if (rule.minLength && stringValue.length < rule.minLength) {
      errors.value[field] = rule.minLengthMessage || `至少需要${rule.minLength}个字符`
      return false
    }

    // 最大长度验证
    if (rule.maxLength && stringValue.length > rule.maxLength) {
      errors.value[field] = rule.maxLengthMessage || `不能超过${rule.maxLength}个字符`
      return false
    }
  ...
  const validateForm = () => {
    ...
    if (formData.paper_type === 'published' && formData.author_names) {
      const authors = formData.author_names.split(',').map(name => name.trim()).filter(name => name.length > 0)
      if (authors.length > 1) {
        // 如果有多个作者，检查是否设置了贡献比例
        const contributions = formData.author_contribution_ratios || []
        const totalContribution = contributions.reduce((sum, ratio) => sum + (ratio || 0), 0)
  ...
  const isValidForm = computed(() => {
    ...
    // 基本必填字段检查
    if (!formData.title?.trim() || !formData.paper_type || !formData.author_names?.trim()) {
      return false
    }
