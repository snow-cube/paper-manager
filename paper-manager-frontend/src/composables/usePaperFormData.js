...
export function usePaperFormData(form, file, authorContributions) {
  ...
  const processAuthors = (authorNames, paperType) => {
    ...
    const authors = authorNames.split(',').map(name => name.trim()).filter(name => name.length > 0)
  ...
  const processKeywords = (keywordNames) => {
    ...
    return keywordNames.split(',').map(k => k.trim()).filter(k => k.length > 0)
  }

  // 转换分类ID
  const processCategories = (categoryIds, paperType) => {
    if (paperType === 'published') {
      return Array.isArray(categoryIds) ? categoryIds : (categoryIds ? [categoryIds] : [])
    } else {
  ...
  const preparePaperData = (form, authorContributions) => {
    return {
      ...
      author_contribution_ratios: authorContributions.some(contrib => contrib > 0)
        ? [...authorContributions]
        : undefined,
      corresponding_author_name: form.corresponding_author_name || null,
    }
  }
  ...
  const handleSubmit = async (props, isEdit) => {
    ...
    try {
      ...
      if (paperType === 'published') {
        // 处理发表论文
        const paperData = preparePaperData(form.value, authorContributions?.value || [])
        ...
        // 处理文献引用
        const referenceData = prepareReferenceData(form.value, currentTeam?.value, currentUser?.value)
