...
export const handlers = [
  ...
  http.get("http://localhost:8000/api/paper", async ({ request }) => {
    ...
    if (paperType) {
      filteredPapers = filteredPapers.filter(
        (paper) => paper.paper_type === paperType
      );
    }
    ...
    if (categoryId) {
      filteredPapers = filteredPapers.filter(
        (paper) => paper.category_id === parseInt(categoryId)
      );
    }
  ...
  // 更新论文
  http.put(
    "http://localhost:8000/api/paper/:id",
  ...
  http.put(
    ...
    async ({ params, request }) => {
      ...
      if (!updatedCategory) {
        return HttpResponse.json({ error: "分类未找到" }, { status: 404 });
      }
